...
router.post('/checkin', (req, res) => {
  ...
  const rec = existing || { email, name, date: today, checkIn: null, checkOut: null, status: 'Present' };
...
router.post('/checkout', (req, res) => {
  ...
  if (!existing?.checkIn) return res.status(404).json({ error: 'No check-in found for today' });
